...
data FileCommand = Cat String
								 | Copy String String
								 | Exit

parseWordsFileCommands : List String -> Maybe FileCommand
parseWordsFileCommands ("cat" :: filename :: []) = Just $ Cat filename
parseWordsFileCommands ("copy" :: source :: destination :: []) = Just $ Copy source destination
parseWordsFileCommands ("exit" :: []) = Just $ Exit
parseWordsFileCommands _ = Nothing

parseFileCommands : String -> Maybe FileCommand
parseFileCommands command = parseWordsFileCommands $ words command

readingFile : (file : Either FileError String) -> String
readingFile (Left error) = "Can not read file because of " ++ show error ++ "\n"
readingFile (Right content) = content

writingFile : (file : Either FileError ()) -> Command ()
writingFile (Left error) = PutStr $ "Can not read file because of " ++ show error ++ "\n"
writingFile (Right ()) = Pure ()

fileCommandToCommand : (fileCommand : FileCommand) -> Command ()
fileCommandToCommand (Cat fileName) = do
	file <- ReadFile fileName
	PutStr $ readingFile file
fileCommandToCommand (Copy source destination) = do
	file <- WriteFile source destination
	writingFile file
fileCommandToCommand Exit = Pure ()

shellCommand : String -> Maybe (Command ())
shellCommand command =
	case parseFileCommands command of
		Just fileCommand => Just $ fileCommandToCommand fileCommand
		Nothing => Nothing
